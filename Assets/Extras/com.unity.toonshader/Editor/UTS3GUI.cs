...
namespace UnityEditor.Rendering.Toon {
    internal partial class UTS3GUI : UnityEditor.ShaderGUI {
        ...
        protected MaterialProperty rad = null;
        protected MaterialProperty rMin = null;
        protected MaterialProperty rMax = null;
        protected MaterialProperty normal = null;
        ...
        void FindProperties(MaterialProperty[] props) {
            ...
            rad = FindProperty("_Rad", props);
            rMin = FindProperty("_Range_Min", props);
            rMax = FindProperty("_Range_Max", props);
...
//- --------------------------------------------------------------------------------------------------------------------
        ...
        void GUI_BasicThreeColors(Material material) {
            ...
            // rMax = FindProperty("_Range_Max", props);
            // normal = FindProperty("_Normal", props);
            var ret = material.GetVector("_Normal");
            EditorGUI.BeginChangeCheck();
            ret = EditorGUILayout.Vector3Field(new GUIContent("Normal/Position"), ret, null);

            if (EditorGUI.EndChangeCheck()) {
                m_MaterialEditor.RegisterPropertyChangeUndo("_Range_Min");
                material.SetVector("_Normal", ret);
            }
            GUI_FloatProperty(material, new FloatProperty(label: "min",
                tooltip: "Sets the reference speed for color shift. When the value is 1, one cycle should take around 6 seconds.",
                propName: "_Range_Min", defaultValue: 0));
            GUI_FloatProperty(material, new FloatProperty(label: "width",
                tooltip: "Sets the reference speed for color shift. When the value is 1, one cycle should take around 6 seconds.",
                propName: "_Range_Max", defaultValue: 0));
            GUI_Toggle(material, new GUIContent("Toggle Linear/Radial"), "_Rad",
                MaterialGetInt(material, "_Rad") != 0);
