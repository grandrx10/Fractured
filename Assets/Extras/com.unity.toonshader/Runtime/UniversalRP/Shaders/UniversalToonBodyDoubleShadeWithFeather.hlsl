...
////// Lighting:
        ...
        // [Toggle()] _Rad("Toggle (Linear/Radial)", float) = 0
                ...
                if (!_Rad)
                {
                    float dist = dot(normalize(_Normal.xyz), i.positionLS.xyz);
                    t = Remap(_Range_Min, _Range_Min+_Range_Max, 0, 1, dist);
                }
                else
                {
                    float dist = distance(_Normal, i.positionLS.xyz);
                    t = Remap(_Range_Min, _Range_Min+_Range_Max, 0, 1, dist);
                }
                ...
                Set_BaseColor = lerp(Set_BaseColor, Set_1st_ShadeColor, 1-saturate(t)) * (1 - smoothstep( 0.4, .2, _HalfLambert_var)*.7);
                Set_1st_ShadeColor = Set_BaseColor * Set_2nd_ShadeColor;
