...
public class PillarSliceable : Slice, ICuttable
{
    ...
    public void Cut(Vector3 cutNormal, Vector3 cutPosition)
    {
        ...
        ShrinkAndDestroy shrinkComponent = rootObj.GetComponent<ShrinkAndDestroy>();
        
        if (shrinkComponent == null)
        {
            shrinkComponent = rootObj.AddComponent<ShrinkAndDestroy>();
            shrinkComponent.shrinkDuration = shrinkDuration;
            shrinkComponent.delayBeforeShrink = delayBeforeShrink;
            shrinkComponent.StartShrinking();
        }
    }
}
